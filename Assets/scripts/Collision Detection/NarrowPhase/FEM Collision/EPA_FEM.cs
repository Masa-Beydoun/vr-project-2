...
public static class EPA_FEM
{
    ...
    public static bool Expand(FEMController objA, FEMController objB, List<Vector3> initialSimplex,
    ...
    private static Vector3 GetFurthestPoint(FEMController obj, Vector3 dir)
    {
        float maxDot = float.MinValue;
        Vector3 best = Vector3.zero;
        foreach (var node in obj.GetAllNodes())
        {
            float dot = Vector3.Dot(node.Position, dir);
            if (dot > maxDot)
            {
                maxDot = dot;
                best = node.Position;
